...
.\gradlew.bat :projects:bot-api:bootJar :projects:bot-main:bootJar | Out-Host
...
docker compose up -d --build postgres rabbitmq bot-api bot-main control-panel | Out-Host
...
if (-not ($bots | Where-Object { $_.botId -eq "bot-main" })) {
    throw "bot-main not found in bot list"
}
...
$startResponse = Invoke-WithRetry -Action {
    Invoke-RestMethod -Uri "http://localhost:3000/api/bots/bot-main/lifecycle" -Method Post -ContentType "application/json" -Body $payload -TimeoutSec 10
...
for ($i = 1; $i -le 20; $i++) {
    ...
    $bot = $bots | Where-Object { $_.botId -eq "bot-main" }
...
if (-not $statusReached) {
    throw "Live status START was not observed for bot-main"
}
